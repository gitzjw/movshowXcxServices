...
//自定义打印类 by jw
#$type ture 则js输出
function dd_log($_, $type,$exit)
{
    if (empty($type)) {
        if (is_bool($_) || is_null($_)) {
            echo '<pre/>';
            var_export($_);
        } elseif (empty($_)) {
            echo '<pre/>';
        ...
        $_ = json_encode($_);
        echo "<script>console.log($_)</script>";
        if(empty($exit)) {
            exit;
        }
    }
...
/*
 * 自定义API接口返回json数据 by jw
 */
function printJson($status, $code, $msg, $data)
{
